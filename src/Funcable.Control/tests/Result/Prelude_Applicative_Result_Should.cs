...
public class Prelude_Applicative_Result_Should
{
	...
	public void Throw_PatternNotHandledException_When_IResult_U_Pattern_Not_Handled_1() =>
		new Action(() => Map(Ok<string, int>(HelloWorld), new TestOk<int, int>(), ToDouble))
			.Should()
			.Throw<InvalidPatternException>()
			.WithMessage("IResult`2.Match: TestOk`2");
	...
	public void Throw_PatternNotHandledException_When_IResult_T_Pattern_Not_Handled_2() =>
		new Action(() => Map(new TestOk<string, int>(), Ok<int, int>(FortyTwo), Ok<double, int>(Pi), ToChar))
			.Should()
			.Throw<InvalidPatternException>()
			.WithMessage("IResult`2.Match: TestOk`2");
	...
	public void Throw_PatternNotHandledException_When_IResult_U_Pattern_Not_Handled_2() =>
		new Action(() => Map(Ok<string, int>(HelloWorld), Ok<int, int>(FortyTwo), new TestOk<double, int>(), ToChar))
			.Should()
			.Throw<InvalidPatternException>()
			.WithMessage("IResult`2.Match: TestOk`2");
}
